...
def average_number_increments_per_insertion(n_hashes, n_requests, n_counters):
    """
    The average number of increments per insertion is independent of the number of counters
    for M>>k and depends only on the number of hash functions used. 
    It is computed by considering the case where all k bins have the same chances of being selected. 
    """
    bins = [0] * n_counters
    number_increments = 0
...
def config_n_counters(n_items,alpha,n_hashes,n_cache_slots,g_k):
    """
    For a given number of hash functions, this function returns the necessary memory
    in order to be above the noisy region. It simply looks for the value of M that verifies
    that the rate of item C is higher than the upper bound on the Error Floor. 
    """
    constant = constant_zipf(n_items,alpha)
    ...
    while max_n_counters > min_n_counters+1:
        ...
        ef = upper_bound_error_floor_rate(
            n_items,
            alpha,
            current_n_counters,
            n_hashes,
...
def average_n_ghosts(n_items, n_counters, n_hashes, n_cache_slots): 
    """
    The following function is nothing but the false positive rate in a regular
    bloom filter where C items are inserted multiplies by the number of items
    other than the C most popular ones. 
    """
...
def config_sketch(n_items, alpha, n_cache_slots, thres_av_n_ghosts, g, n_hashes_max):
    """
    The following function is the final one, returns the number of counters and
    the number of hash functions in order to distinguish C - thres_av_n_ghosts
    on average. 
    """
    n_hashes = 2
